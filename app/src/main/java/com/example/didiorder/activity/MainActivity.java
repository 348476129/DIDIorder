...
public class MainActivity extends AppCompatActivity implements IMainView {
    ...
    private void initData() {
        ...
        fragmentAll = ijob.getFragment()[0];
        fragmentFinish = ijob.getFragment()[2];
        fragmentUnder = ijob.getFragment()[1];
    }
    ...
    private void initClick() {
        ...
        swipeRefreshLayout.setOnRefreshListener(()->{
            mainActivityPresenter.setPage(1);
            mainActivityPresenter.getOrder();
        });
        fragmentFinish.setPresenterListener(new FragmentOrder.PresenterListener() {
            @Override
            public void OnUpDataList() {
            }
        });
        fragmentAll.setPresenterListener(new FragmentOrder.PresenterListener() {
            @Override
            public void OnUpDataList() {
            }
        });
        fragmentUnder.setPresenterListener(new FragmentOrder.PresenterListener() {
            @Override
            public void OnUpDataList() {
            }
        });
        fragmentFinish.setFragmentScrollListener(new FragmentOrder.FragmentScrollListener() {
            @Override
            public void OnTop(boolean isTop) {
                swipeRefreshLayout.setEnabled(isTop);
            }
        });
        fragmentAll.setFragmentScrollListener(new FragmentOrder.FragmentScrollListener() {
            @Override
            public void OnTop(boolean isTop) {
                swipeRefreshLayout.setEnabled(isTop);
            }
        });
        fragmentUnder.setFragmentScrollListener(new FragmentOrder.FragmentScrollListener() {
            @Override
            public void OnTop(boolean isTop) {
                swipeRefreshLayout.setEnabled(isTop);
            }
    ...
    @Override
    public void setAdapter(List<AllOrder> orderLista, List<AllOrder> orderListb, List<AllOrder> orderListc) {

        ijob.setAdapter(context, orderLista, orderListb, orderListc);
    }

    @Override
    public void showError(String s) {
        if (floatingActionButton.getVisibility() == View.VISIBLE) {
            Snackbar.make(floatingActionButton, s, Snackbar.LENGTH_SHORT).setAction("重试", v -> {
                mainActivityPresenter.getOrder();
            }).show();
        } else {
            Snackbar.make(toolbar, s, Snackbar.LENGTH_SHORT).setAction("重试", v -> {
                mainActivityPresenter.getOrder();
            }).show();
        }
    }
    ...
    public void addAdapter(List<AllOrder> orderLista, List<AllOrder> orderListb, List<AllOrder> orderListc) {
        ijob.addAdapter(orderLista, orderListb, orderListc);
    }

    @Override
    public void isSwipe(boolean swipe) {
        swipeRefreshLayout.setEnabled(!swipe);
        if (swipe==false){
